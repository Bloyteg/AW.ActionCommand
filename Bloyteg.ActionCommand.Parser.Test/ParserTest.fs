...
module ParserTest =
    ...
    let ``commmand ignores known commands``() =
        ...
        | [Create([IgnoredCommand(_)])] -> passTest()
        | _ -> failTest()
    ...
    let ``move should allow same parameter twice, taking only the final occurrence`` () =
        ...
        | [Create([Move((1.0, None, None), (Some(2.0), _, _, _, _, _, _, _, _), _)])] -> passTest()
        | _ -> failTest()
    ...
    let ``shear requires at least one element`` () =
        ...
        | [Create([Shear((1.0, None, None, None, None, None), None, None)])] -> passTest()
        | _ -> failTest()

    [<Test>]
    let ``shear allows two elements`` () =
        match Parser.parse "create shear 1 1" with
        | [Create([Shear((1.0, Some 1.0, None, None, None, None), None, None)])] -> passTest()
        | _ -> failTest()

    [<Test>]
    let ``shear allows three elements`` () =
        match Parser.parse "create shear 1 1 1" with
        | [Create([Shear((1.0, Some 1.0, Some 1.0, None, None, None), None, None)])] -> passTest()
        | _ -> failTest()

    [<Test>]
    let ``shear allows four elements`` () =
        match Parser.parse "create shear 1 1 1 1" with
        | [Create([Shear((1.0, Some 1.0, Some 1.0, Some 1.0, None, None), None, None)])] -> passTest()
        | _ -> failTest()

    [<Test>]
    let ``shear allows five elements`` () =
        match Parser.parse "create shear 1 1 1 1 1" with
        | [Create([Shear((1.0, Some 1.0, Some 1.0, Some 1.0, Some 1.0, None), None, None)])] -> passTest()
        | _ -> failTest()

    [<Test>]
    let ``shear allows six elements`` () =
        match Parser.parse "create shear 1 1 1 1 1 1" with
        | [Create([Shear((1.0, Some 1.0, Some 1.0, Some 1.0, Some 1.0, Some 1.0), None, None)])] -> passTest()
        | _ -> failTest()

    [<Test>]
    let ``shear allows name parameter`` () =
        match Parser.parse "create shear 1 name=test" with
        | [Create([Shear((1.0, None, None, None, None, None), Some "test", None)])] -> passTest()
        | _ -> failTest()
    ...
    let ``skew requires at least one element`` () =
        ...
        | [Create([Skew((1.0, None, None, None, None, None), None, None)])] -> passTest()
        | _ -> failTest()

    [<Test>]
    let ``skew allows two elements`` () =
        match Parser.parse "create skew 1 1" with
        | [Create([Skew((1.0, Some 1.0, None, None, None, None), None, None)])] -> passTest()
        | _ -> failTest()

    [<Test>]
    let ``skew allows three elements`` () =
        match Parser.parse "create skew 1 1 1" with
        | [Create([Skew((1.0, Some 1.0, Some 1.0, None, None, None), None, None)])] -> passTest()
        | _ -> failTest()

    [<Test>]
    let ``skew allows four elements`` () =
        match Parser.parse "create skew 1 1 1 1" with
        | [Create([Skew((1.0, Some 1.0, Some 1.0, Some 1.0, None, None), None, None)])] -> passTest()
        | _ -> failTest()

    [<Test>]
    let ``skew allows five elements`` () =
        match Parser.parse "create skew 1 1 1 1 1" with
        | [Create([Skew((1.0, Some 1.0, Some 1.0, Some 1.0, Some 1.0, None), None, None)])] -> passTest()
        | _ -> failTest()

    [<Test>]
    let ``skew allows six elements`` () =
        match Parser.parse "create skew 1 1 1 1 1 1" with
        | [Create([Skew((1.0, Some 1.0, Some 1.0, Some 1.0, Some 1.0, Some 1.0), None, None)])] -> passTest()
        | _ -> failTest()

    [<Test>]
    let ``skew allows name parameter`` () =
        match Parser.parse "create skew 1 name=test" with
        | [Create([Skew((1.0, None, None, None, None, None), Some "test", None)])] -> passTest()
        | _ -> failTest()
    ...
    let ``color allows tint parameter before color name with name parameter first`` () =
        ...
        | [Create([Color((Some "red", Some "test", Some ()), None)])] -> passTest()
        | _ -> failTest()
    ...
    let ``picture allows url parameter`` () =
        ...
        | [Create([Picture((Some uri, None, None, None), _)])] when uri.AbsoluteUri = "http://www.test.com/test.jpg" -> passTest()
        | _ -> failTest()
    ...
    let ``picture allows url parameter before update parameter`` () =
        ...
        | [Create([Picture((Some uri, Some 60, None, None), _)])] when uri.AbsoluteUri = "http://www.test.com/test.jpg" -> passTest()
        | _ -> failTest()
    ...
    let ``picture allows url parameter after update parameter followed by mip parameter`` () =
        ...
        | [Create([Picture((Some uri, Some 60, Some true, None), _)])] when uri.AbsoluteUri = "http://www.test.com/test.jpg" -> passTest()
        | _ -> failTest()
    ...
    let ``visible allows for false value with name`` () =
        ...
        | [Create([Visible((false, Some "test", None), _)])] -> passTest()
        | _ -> failTest()
    ...
    let ``visible allows for false value with name and radius before flag`` () =
        ...
        | [Create([Visible((false, Some "test", Some 50), _)])] -> passTest()
        | _ -> failTest()
    ...
    let ``at trigger with VRT parameter and no hour should yield At case with VRTTimer`` () =
        ...
        | [At(VRTTimer(None, Some(0), Some(0)), _)] -> passTest()
        | _ -> failTest() 
    ...
    let ``at trigger with VRT parameter and no hour and minute should yield At case with VRTTimer`` () =
        ...
        | [At(VRTTimer(None, None, Some(0)), _)] -> passTest()
        | _ -> failTest()
