...
module ParserHelpersTests =    
    ...
    let ``unorderedOpt2 returns two items that exist`` () =
        ...
        | Success(result, _, _) -> match result with
                                   | (Some("hello"), Some("world")) -> passTest()
                                   | _ -> failTest()
        | _ -> failTest()
    ...
    let ``parse valid uri`` () =
        ...
        | Success(result, _, _) -> if result.AbsoluteUri = "http://test.com/" then passTest() else failTest()
        | _ -> failTest()
    ...
    let ``parse valid uri with query string`` () =
        ...
        | Success(result, _, _) -> if result.AbsoluteUri = "http://test.com/?x=5&q=ss%20&t=5eB34" then passTest() else failTest()
        | _ -> failTest()

    [<Test>]
    let ``parse invalid url`` () =
        match run (puri) "name=test.com" with
        | Success(_, _, _) -> failTest()
        | _ -> passTest()
    ...
    let ``takeUntilWithEscape skips over invalid char inside escape sequence`` () =
        ...
        | Success("test\"test,test\"", _, _) -> passTest();
        | _ -> failTest()
    ...
    let ``takeUntilWithEscape skips over invalid char inside escape sequence, but stops after escape sequence`` () =
        ...
        | Success("test\"test,test\"test", _, _) -> passTest();
        | _ -> failTest()
